...
<head>
  ...
  <title><?php echo $nav; ?> Change Logo</title>
...
?>
        ...
        <div class="col-md-16">
                ...
                <div class="col-sm-6">
                    <label for="navbarlogo">Logo</label>
                    <input type="file" name="navbarlogo" id="navbarlogo" onchange="readURL(this);" class="form-control-file">
                    </div>
                </div>
                
                <img width="250px" height="250px" id="newlogo1">
                <br>
                 <br>
              
                            <input type="submit" name= "updatelogo" id="updatelogo" value="Update Logo" class="form-control btn btn-block btn-primary">
                        <br>
                        </form>
...
if (isset($_POST['updatelogo'])) {
  $id=$_POST['id'];
  if(!empty($_FILES["navbarlogo"]["name"])) { 
    // Get file info 
    $fileName1 = basename($_FILES["navbarlogo"]["name"]); 
    $fileType1 = pathinfo($fileName1, PATHINFO_EXTENSION);

    $allowTypes = array('jpg','png','jpeg','gif'); 

    if(in_array($fileType1, $allowTypes)){ 
      $image1 = $_FILES['navbarlogo']['tmp_name']; 
...
if ($conn->query($sql) === TRUE) {
  ?>
  <script>
   swal("Success!", "Logo updated successfully!","success").then( () => {
     location.href = 'newlogo.php'
 })   
             </script>
